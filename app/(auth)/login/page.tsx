    ...
    <div style={{ minHeight: '100vh', background: 'var(--surface)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '24px' }}>
      <div style={{ width: '100%', maxWidth: '400px' }}>
        ...
        <div style={{ textAlign: 'center', marginBottom: '32px' }}>
          ...
          <div style={{ color: 'var(--ink4)', fontSize: '13px', marginTop: '4px' }}>Entre na sua conta</div>
        </div>
        ...
        <div style={{ background: 'var(--card)', borderRadius: 'var(--r-lg)', border: '1px solid var(--border)', padding: '28px' }}>
          <form onSubmit={handleLogin} style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
            ...
            <div>
              <label style={{ display: 'block', fontSize: '11px', fontWeight: 700, color: 'var(--ink4)', marginBottom: '5px', letterSpacing: '.3px' }}>SENHA</label>
              <input
                type="password"
                value={senha}
            ...
            <div style={{ textAlign: 'right' }}>
              <Link href="/reset-senha" style={{ fontSize: '12px', color: 'var(--amber)', fontWeight: 600, textDecoration: 'none' }}>Esqueceu a senha?</Link>
            </div>
            <button
              type="submit"
              disabled={loading}
        ...
        <p style={{ textAlign: 'center', marginTop: '20px', fontSize: '13px', color: 'var(--ink3)' }}>
          Não tem conta?{' '}
          <Link href="/cadastro" style={{ color: 'var(--amber)', fontWeight: 700, textDecoration: 'none' }}>Criar conta</Link>
        </p>
      </div>
    </div>
  )
